...
const cityData: Record<string, { name: string; state: string; zones: Record<string, { lat: number; lng: number; risk: number; pin: string; stores: { id: string; name: string; platform: string; lat: number; lng: number }[] }> }> = {
  mumbai: {
    name: 'Mumbai', state: 'Maharashtra',
    zones: {
      'Andheri West': { lat: 19.1360, lng: 72.8296, risk: 65, pin: '400053', stores: [
      ...
      'Bandra East': { lat: 19.0596, lng: 72.8511, risk: 45, pin: '400051', stores: [
      ...
      'Powai': { lat: 19.1176, lng: 72.9060, risk: 35, pin: '400076', stores: [
      ...
      'Malad West': { lat: 19.1872, lng: 72.8484, risk: 72, pin: '400064', stores: [
  ...
  chennai: {
    ...
    zones: {
      'T. Nagar': { lat: 13.0418, lng: 80.2341, risk: 40, pin: '600017', stores: [
      ...
      'Adyar': { lat: 13.0067, lng: 80.2572, risk: 55, pin: '600020', stores: [
      ...
      'Velachery': { lat: 12.9815, lng: 80.2180, risk: 68, pin: '600042', stores: [
      ...
      'Anna Nagar': { lat: 13.0850, lng: 80.2101, risk: 38, pin: '600040', stores: [
      ...
      'OMR (Sholinganallur)': { lat: 12.9010, lng: 80.2279, risk: 48, pin: '600119', stores: [
  ...
  kolkata: {
    ...
    zones: {
      'Salt Lake': { lat: 22.5800, lng: 88.4179, risk: 58, pin: '700091', stores: [
      ...
      'New Town': { lat: 22.5926, lng: 88.4845, risk: 42, pin: '700156', stores: [
      ...
      'Gariahat': { lat: 22.5170, lng: 88.3690, risk: 62, pin: '700019', stores: [
      ...
      'Howrah': { lat: 22.5958, lng: 88.2636, risk: 70, pin: '711101', stores: [
...
export async function seed(knex: Knex): Promise<void> {
  ...
  await knex('device_fingerprints').del();
  await knex('worker_location_log').del();
  await knex('audit_log').del();
  await knex('financial_ledger').del();
  await knex('payouts').del();
  await knex('claims').del();
  await knex('disruption_events').del();
  await knex('policies').del();
  await knex('otp_records').del();
  await knex('workers').del();
  await knex('dark_stores').del();
  await knex('delivery_zones').del();

  // Insert zones and stores
  for (const [, cityInfo] of Object.entries(cityData)) {
    for (const [zoneName, zone] of Object.entries(cityInfo.zones)) {
      const [zoneRow] = await knex('delivery_zones')
        .insert({
          pin_code: zone.pin,
          name: zoneName,
          city: cityInfo.name,
  ...
  console.log('Seeded delivery zones and dark stores');
}
